...
async function load() {
  loading.value = true
  try { const res = await payrollCnApi.payslips(); payslips.value = res.data.data?.items || []; total.value = payslips.value.length } finally { loading.value = false }
}

async function sendEmail(id: string) {
  try { await payrollCnApi.emailPayslip(id); ElMessage.success(t('action.email_sent')) } catch (e: any) { ElMessage.error(e.message) }
}
...
<template>
  <div class="page-container">
    <div class="page-header"><h3>{{ t('payroll.cn.payslips') }}</h3></div>
