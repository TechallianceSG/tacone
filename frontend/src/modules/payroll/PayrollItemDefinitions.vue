...
async function load() { loading.value = true; try { const res = await api.itemDefinitions(); items.value = res.data.data || [] } catch (e: any) { ElMessage.error(e.message) } finally { loading.value = false } }
function openEdit(row: any) { form.value = { ...row }; dialogVisible.value = true }
async function save() { saving.value = true; try { await api.saveItemDefinition(form.value); dialogVisible.value = false; ElMessage.success(t('action.saved')); await load() } catch (e: any) { ElMessage.error(e.message) } finally { saving.value = false } }
