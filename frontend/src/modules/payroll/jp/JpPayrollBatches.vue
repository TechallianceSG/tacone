...
async function loadEntities() { try { const res = await payrollJpApi.entities(); entities.value = res.data?.data || [] } catch (_) {} }

function entityLabel(id: string) {
  const f = entities.value.find((e: any) => e.entity_id === id)
  if (!f) return id
  return `${f.entity_code || f.entity_id} - ${f.entity_name || ''} (${f.country || ''})`
}
...
async function confirmVoid() {
  voiding.value = true
  try {
    await payrollJpApi.voidSheet(voidSheetId.value, { reason: voidReason.value })
...
async function deleteSheet(id: string) {
  ...
  try {
    await payrollJpApi.deleteSheet(id)
      ...
      <el-table :data="paged" v-loading="loading" border stripe size="small" style="width:100%">
        ...
        <el-table-column :label="t('field.actions')" min-width="200" fixed="right">
          <template #default="{row}">
            ...
            <el-button v-if="row.status==='voided'" size="small" text type="danger" @click="deleteSheet(row.batch_id)">{{ t('action.delete') }}</el-button>
          </template>
        </el-table-column>
      </el-table>
