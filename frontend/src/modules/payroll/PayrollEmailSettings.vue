...
const { t } = useI18n()
const { api } = usePayroll(props.countryCode)
...
async function load() {
  ...
  try {
    // Check SMTP status
    try {
      const st = await api.smtpStatus()
      smtpAvailable.value = st.data?.configured || false
    } catch { smtpAvailable.value = false }

    // Load item definitions
    try {
      const itemsRes = await api.itemDefinitions()
      allItems.value = (itemsRes.data?.data || itemsRes.data || []) as ItemDef[]
    } catch { allItems.value = [] }

    // Load saved settings
    const res = await api.emailSettings({ country_code: props.countryCode.toUpperCase() })
    ...
    try {
      testEmail.value = localStorage.getItem(`tacai_pay_${props.countryCode}_test_email`) || ''
...
async function sendTest() {
  ...
  // Persist test email
  try { localStorage.setItem(`tacai_pay_${props.countryCode}_test_email`, testEmail.value.trim()) } catch {}

  testing.value = true; testResult.value = null
  try {
    await api.testEmailSettings({ test_to: testEmail.value.trim(), country_code: props.countryCode.toUpperCase() })
