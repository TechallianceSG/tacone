...
const { t } = useI18n()
const { api, baseRoute, fmtCurrency } = usePayroll(props.countryCode)
...
async function load() {
  ...
  try {
    ...
    const res = await api.payslips(params)
...
async function sendEmail(record: any) {
  ...
  try {
    await api.sendSinglePayslip(id)
...
async function sendAll() {
  ...
  try {
    ...
    const res = await api.sendPayslipsAll(payload)
...
async function resendFailed() {
  ...
  try {
    const res = await api.sendPayslipsSelected({ record_ids: failedIds })
    const data = res.data?.data || res.data || {}
    progressData.value.sent = data.sent || 0
    progressData.value.failed = data.failed || 0
    progressData.value.results = data.results || []
...
async function viewPayslip(record: any) {
  ...
  previewHtml.value = ''
  try {
    const res = await api.viewPayslipHtml(id)
...
async function openLogDialog() {
  ...
  try {
    const res = await api.emailLogs({ limit: 500 })
...
onMounted(async () => {
  ...
  // Check SMTP status
  try {
    const res = await api.smtpStatus()
    ...
    <div class="fiori-card">
      ...
      >
        ...
        <el-table-column prop="gross_pay" :label="t('field.gross_pay')" min-width="130" align="right">
          <template #default="{row}">{{ row.gross_pay ? fmtCurrency(row.gross_pay) : '-' }}</template>
        </el-table-column>
        <el-table-column prop="net_pay" :label="t('field.net_pay')" min-width="130" align="right">
          <template #default="{row}"><strong style="color:#1B6CB2">{{ row.net_pay ? fmtCurrency(row.net_pay) : '-' }}</strong></template>
        </el-table-column>
