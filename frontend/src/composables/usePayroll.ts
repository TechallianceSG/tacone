import { payrollJpApi, payrollSgApi } from '@/api/client'
...
export function usePayroll(countryCode: CountryCode) {
  ...
  // API client
  const api = isJP ? payrollJpApi : payrollSgApi

  // Route base
  const baseRoute = `/payroll/${countryCode}`

  // Status config
  const STATUS_CONFIG = isJP ? JP_STATUS_CONFIG : SG_STATUS_CONFIG
  const STATUS_WORKFLOW = isJP ? JP_STATUS_WORKFLOW : SG_STATUS_WORKFLOW
  const statusIndex = isJP ? jpStatusIndex : sgStatusIndex

  // Action labels
  const ACTION_LABELS = isJP ? JP_ACTION_LABELS : SG_ACTION_LABELS
  const actionLabel = isJP ? jpActionLabel : sgActionLabel
  const parseAuditValue = isJP ? parseJpAuditValue : parseSgAuditValue

  // Item categories (same for both countries)
  ...
  // Currency formatting
  const currencySymbol = isJP ? '¥' : 'SGD '
  function fmtCurrency(v: number): string {
    if (!v && v !== 0) return '-'
    if (isJP) return `¥${Number(v).toLocaleString()}`
    return `SGD ${Number(v).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
  }
