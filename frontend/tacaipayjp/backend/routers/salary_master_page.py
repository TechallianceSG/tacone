...
.import-result{background:#e8f5e9;border:1px solid #a5d6a7;border-radius:10px;padding:12px 16px;margin-top:12px;font-size:0.95rem;color:#2e7d32;display:none}
.modal{display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.45);z-index:9999;align-items:center;justify-content:center}
.modal.show{display:flex}
.modal-content{background:var(--surface);border-radius:16px;padding:28px 32px;min-width:380px;max-width:500px;box-shadow:0 12px 40px rgba(0,0,0,0.2);animation:modalIn .2s ease}
@keyframes modalIn{from{opacity:0;transform:translateY(-20px)}to{opacity:1;transform:translateY(0)}}
.modal-content h3{font-size:1.2rem;margin-bottom:12px;color:var(--navy);display:flex;align-items:center;gap:8px}
.modal-content p{font-size:1.02rem;color:var(--text);margin-bottom:24px;line-height:1.6}
.modal-actions{display:flex;gap:12px;justify-content:flex-end}
.modal-actions .btn{padding:10px 24px;font-size:1rem}
.import-count-badge{display:inline-block;background:var(--accent);color:#fff;padding:2px 10px;border-radius:999px;font-weight:800;font-size:0.95rem}
</style>"""
...
async def salary_master_import(request: Request):
    """Import page — filterable employee list with confirmation modal."""
    lang = get_lang(request)
    msgs = load_i18n(lang)

    # Load masterdata from JSON (authoritative source for departments/teams)
    import json, os
    base_dir = os.path.join(os.path.dirname(__file__), '..', '..', '..', '..')
    masterdata_dir = os.path.join(base_dir, 'TACAI-Core', 'masterdata', 'database')
    ...
    # Load entities: try PG first, fallback to JSON
    try:
        pg_entities = fetch_all("SELECT entity_id, entity_code, entity_name_ja, entity_name_en, country FROM md_entities WHERE status='active' ORDER BY entity_id")
    except Exception:
        pg_entities = None

    if pg_entities:
        entity_list = pg_entities
    else:
        entity_list = [e for e in _load_json('entities.json') if e.get('status') == 'active']

    # Load departments from JSON (PG may be empty)
    dept_list = [d for d in _load_json('departments.json') if d.get('status') == 'active']
    # Load teams from JSON (PG may be empty)
    team_list = [t for t in _load_json('teams.json') if t.get('status') == 'active']

    # Build entity map for display labels
    entity_map = {}
    for e in entity_list:
        code = e.get('entity_code', '')
        name = e.get('entity_name_ja', e.get('entity_name_en', ''))
        country = e.get('country', '')
        entity_map[e['entity_id']] = f"{code} - {name} ({country})"

    # Entity dropdown HTML with labels, default ENT-0001 (Japan)
    JAPAN_ENTITY = 'ENT-0001'
    ent_label_opts = '<option value="">' + t(msgs, 'general.all_entities') + '</option>'
    for e in entity_list:
        eid = e['entity_id']
        label = entity_map.get(eid, eid)
        sel = ' selected' if eid == JAPAN_ENTITY else ''
        ent_label_opts += f'<option value="{eid}"{sel}>{label}</option>'

    # Fetch employees: default Japan entity
    try:
        ea = fetch_all(
            "SELECT employee_id, employee_number, profile, payroll, employment FROM emp_employees WHERE payroll IS NOT NULL AND employment::jsonb->>'entity_id' = %s ORDER BY employee_number",
            (JAPAN_ENTITY,)
        )
    except Exception:
        ea = []
    ...
    # Initial table rows (Japan entity employees)
    rows_html = _render_employee_rows(ea, existing_ids, msgs)

    # Build JS data structures for cascading filters
    import json as _json
    dept_js = _json.dumps([{
        'department_id': d.get('department_id', ''),
        'department_name': d.get('department_name_ja', d.get('department_name_en', '')),
        'entity_id': d.get('entity_id', ''),
    } for d in dept_list], ensure_ascii=False)
    team_js = _json.dumps([{
        'team_id': t.get('team_id', ''),
        'team_name': t.get('team_name_ja', t.get('team_name_en', '')),
        'department_id': t.get('department_id', ''),
    } for t in team_list], ensure_ascii=False)
...
<select id="filterEntity" onchange="onEntityChange()" style="min-width:220px">{ent_label_opts}</select>
<select id="filterDept" onchange="onDeptChange()"><option value="">{t(msgs,'general.all_depts')}</option></select>
<select id="filterTeam" onchange="applyFilters()"><option value="">{t(msgs,'general.all_teams')}</option></select>
<input id="filterSearch" type="text" placeholder="{t(msgs,'salary_master.import_filter_search')}" style="width:200px" onkeydown="if(event.key==='Enter')applyFilters()">
<button class="btn btn-primary btn-sm" onclick="applyFilters()">{t(msgs,'salary_master.import_btn_filter')}</button>
</div>
...
<button class="btn btn-outline" onclick="document.querySelectorAll('.import-checkbox:not(:disabled)').forEach(cb=>cb.checked=!cb.checked)">☑ {t(msgs,'salary_master.import_toggle_all')}</button>
...
<!-- Employee Table -->
<div class="card"><div style="overflow-x:auto"><table id="employeeTable">
<thead><tr><th style="width:40px"></th><th>社員番号</th><th>氏名</th><th>部署</th><th>チーム</th><th>法人</th><th>種別</th><th>状態</th></tr></thead>
<tbody>{rows_html if rows_html else '<tr><td colspan="7" class="empty">EmployeeAdminにデータがありません</td></tr>'}</tbody>
</table></div></div>
<div class="footer-text" id="footerInfo">{len(ea)} employees | 灰色 = {t(msgs,'salary_master.already_imported')}</div></div>

<!-- Confirmation Modal -->
<div id="confirmModal" class="modal">
<div class="modal-content">
<h3>⚠️ {t(msgs,'salary_master.import_confirm_title')}</h3>
<p id="confirmText">{t(msgs,'salary_master.import_confirm_text', count='0')}</p>
<div class="modal-actions">
<button id="confirmNoBtn" class="btn btn-outline" onclick="cancelImport()">{t(msgs,'salary_master.import_confirm_no')}</button>
<button id="confirmYesBtn" class="btn btn-success" onclick="confirmImport()">{t(msgs,'salary_master.import_confirm_yes')}</button>
</div></div></div>

<script>
// Masterdata for cascading filters
var allDepts = {dept_js};
var allTeams = {team_js};

function renderDeptOptions() {{
  var entityId = document.getElementById('filterEntity').value;
  var sel = document.getElementById('filterDept');
  var currentVal = sel.value;
  sel.innerHTML = '<option value="">{t(msgs,'general.all_depts')}</option>';
  allDepts.forEach(function(d) {{
    if (!entityId || d.entity_id === entityId) {{
      var opt = document.createElement('option');
      opt.value = d.department_id;
      opt.textContent = d.department_name || d.department_id;
      if (d.department_id === currentVal) opt.selected = true;
      sel.appendChild(opt);
    }}
  }});
}}
...
function renderTeamOptions() {{
  var deptId = document.getElementById('filterDept').value;
  var sel = document.getElementById('filterTeam');
  var currentVal = sel.value;
  sel.innerHTML = '<option value="">{t(msgs,'general.all_teams')}</option>';
  allTeams.forEach(function(t) {{
    if (!deptId || t.department_id === deptId) {{
      var opt = document.createElement('option');
      opt.value = t.team_id;
      opt.textContent = t.team_name || t.team_id;
      if (t.team_id === currentVal) opt.selected = true;
      sel.appendChild(opt);
    }}
  }});
}}
...
// Initialize: pre-fill department dropdown for default Japan entity
renderDeptOptions();
renderTeamOptions();
...
async function applyFilters() {{
  ...
  var eid = document.getElementById('filterEntity').value;
  ...
  document.getElementById('status').textContent = '{t(msgs,'salary_master.import_processing')}';
  try {{
    var resp = await fetch('/api/salary-master/importable-employees?' + params.toString());
    var d = await resp.json();
    renderTable(d.data);
    document.getElementById('footerInfo').textContent = d.count + ' employees | 灰色 = {t(msgs,'salary_master.already_imported')}';
...
function renderTable(data) {{
  ...
  if (!data || data.length === 0) {{
    tbody.innerHTML = '<tr><td colspan="7" class="empty">EmployeeAdminにデータがありません</td></tr>';
    return;
  }}
  var html = '';
  data.forEach(function(emp) {{
    if (emp.already_imported) {{
      html += '<tr style="background:#f5f5f5;color:#bbb">' +
        '<td><input type="checkbox" disabled></td>' +
        '<td style="text-decoration:line-through">' + (emp.employee_number||'') + '</td>' +
        '<td style="text-decoration:line-through">' + (emp.employee_name||'') + '</td>' +
        '<td>' + (emp.department_label||'') + '</td>' +
        '<td>' + (emp.team_label||'') + '</td>' +
        '<td>' + (emp.entity_id||'') + '</td>' +
        '<td>' + (emp.salary_type||'monthly') + '</td>' +
        '<td><span class="badge badge-inactive">{t(msgs,'salary_master.already_imported')}</span></td></tr>';
    }} else {{
      html += '<tr>' +
        '<td><input type="checkbox" class="import-checkbox" value="' + emp.employee_id + '"></td>' +
...
async function importSelected() {{
  pendingIds = Array.from(document.querySelectorAll('.import-checkbox:checked')).map(cb => cb.value);
  if (pendingIds.length === 0) {{ alert('{t(msgs,'salary_master.import_select')}'); return; }}
  document.getElementById('confirmText').innerHTML = '{t(msgs,'salary_master.import_confirm_text')}'.replace('{{count}}', '<span class="import-count-badge">' + pendingIds.length + '</span>');
...
async function confirmImport() {{
  ...
  try {{
    var resp = await fetch('/api/salary-master/import-selected', {{
      ...
      body: JSON.stringify({{employee_ids: pendingIds}})
    }});
    var d = await resp.json();
    if (resp.ok) {{
      document.getElementById('status').textContent = '';
      document.getElementById('importResult').style.display = 'block';
      document.getElementById('importResult').innerHTML = '✅ {t(msgs,'salary_master.import_result')}'.replace('{{date}}', d.imported_date).replace('{{time}}', d.imported_time).replace('{{count}}', d.imported).replace('{{user}}', d.imported_by);
      // Refresh table after 1.5s
      setTimeout(function(){{ applyFilters(); }}, 1500);
    }} else {{
...
def _render_employee_rows(ea, existing_ids, msgs):
    """Render table rows for employee list."""
    rows = ""
    for emp in ea:
        ...
        if isinstance(profile, str):
            import json; profile = json.loads(profile)
        employment = emp.get("employment", {})
        if isinstance(employment, str):
            import json; employment = json.loads(employment)
        eid = emp["employee_id"]
        already = eid in existing_ids
        name = profile.get("name", {}).get("display_name", "") if isinstance(profile, dict) else ""
        dept = employment.get("department", "") if isinstance(employment, dict) else ""
        team = employment.get("team_id", "") if isinstance(employment, dict) else ""
        entity = employment.get("entity_id", "") if isinstance(employment, dict) else ""
        st = payroll.get("salary_type", "monthly") if isinstance(payroll, dict) else "monthly"

        if already:
            rows += f"""<tr style="background:#f5f5f5;color:#bbb">
<td><input type="checkbox" disabled></td>
<td style="text-decoration:line-through">{emp.get('employee_number','')}</td>
<td style="text-decoration:line-through">{name}</td>
<td>{dept}</td><td>{team}</td><td>{entity}</td><td>{st}</td>
<td><span class="badge badge-inactive">{t(msgs,'salary_master.already_imported')}</span></td></tr>"""
        else:
            rows += f"""<tr>
