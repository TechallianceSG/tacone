...
async def list_importable_employees(
    ...
    """Return employees from EmployeeAdmin with import status and filters."""
    where = ["payroll IS NOT NULL"]
    params = []

    if entity_id:
        where.append("employment::jsonb->>'entity_id' = %s")
        params.append(entity_id)
    if department_id:
        where.append("employment::jsonb->>'department_id' = %s")
        params.append(department_id)
    if team_id:
        where.append("employment::jsonb->>'team_id' = %s")
        params.append(team_id)
    if q:
        where.append(
            "(employee_number ILIKE %s OR profile::jsonb->'name'->>'display_name' ILIKE %s OR employment::jsonb->>'department' ILIKE %s)"
        )
        params.extend([f"%{q}%"] * 3)

    sql = f"SELECT employee_id, employee_number, profile, payroll, employment FROM emp_employees WHERE {' AND '.join(where)} ORDER BY employee_number"
    ea_employees = fetch_all(sql, tuple(params) if params else None)
...
async def import_selected(request: Request):
    ...
    return {
        ...
        "imported_date": now.strftime("%Y-%m-%d"),
        "imported_time": now.strftime("%H:%M:%S"),
        "imported_by": user.get("display_name", user.get("username", "unknown")),
    }
