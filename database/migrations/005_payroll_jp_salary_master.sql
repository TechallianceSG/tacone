-- TACAI Pay JP — Migration 005: Salary Master table (薪资主数据)
-- Date: 2026-07-01
-- Purpose: Create full pay_jp_salary_master table with all JP payroll fields
...
CREATE TABLE pay_jp_salary_master (
    ...
    transport_allowance NUMERIC(12,2) DEFAULT 0,   -- 交通費
    phone_allowance     NUMERIC(12,2) DEFAULT 0,   -- 電話手当
    performance_bonus   NUMERIC(12,2) DEFAULT 0,   -- 業績賞与
    project_bonus       NUMERIC(12,2) DEFAULT 0,   -- 案件賞与
    ...
    -- Age (for nursing care insurance eligibility)
    age_at_fiscal_year_start INTEGER DEFAULT 0,         -- 年度初めの年齢（介護保険判定用）
...
CREATE INDEX IF NOT EXISTS idx_pjp_sm_employee ON pay_jp_salary_master(employee_id);
CREATE INDEX IF NOT EXISTS idx_pjp_sm_entity ON pay_jp_salary_master(entity_id);
CREATE INDEX IF NOT EXISTS idx_pjp_sm_active ON pay_jp_salary_master(active) WHERE active = true;
CREATE INDEX IF NOT EXISTS idx_pjp_sm_type ON pay_jp_salary_master(salary_type);
CREATE INDEX IF NOT EXISTS idx_pjp_sm_salary_master_id ON pay_jp_salary_master(salary_master_id);
